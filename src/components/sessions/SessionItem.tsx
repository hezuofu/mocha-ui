...
export default function SessionItem({
  ...
  return (
    <div className={`${cls}${swipeX > 0 ? " swiping-right" : swipeX < 0 ? " swiping-left" : ""}`}
      data-sid={session.session_id}
      onClick={onSelect}
    ...
    >
        ...
        <div className="session-title-row">
          {editing ? (
            <input ref={inputRef} className="session-rename-input" value={editTitle}
              ...
              onKeyDown={e => { if (e.key === 'Enter') handleRename(); if (e.key === 'Escape') { setEditing(false); setEditTitle(session.title); } }}
      ...
      <span className="session-attention-indicator session-state-indicator" aria-hidden="true" />

      <div className="session-actions" ref={menuRef}>
        <button type="button" className="session-actions-trigger" title="Conversation actions"
          aria-haspopup="menu" aria-label="Conversation actions"
          onClick={e => { e.stopPropagation(); setMenuOpen(!menuOpen); }}>
          <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" stroke="none" aria-hidden="true"><circle cx="8" cy="3" r="1.25"/><circle cx="8" cy="8" r="1.25"/><circle cx="8" cy="13" r="1.25"/></svg>
        </button>
        {menuOpen && (
          <div className="session-actions-menu">
            ...
            <button onClick={e => { e.stopPropagation(); onDuplicate(); setMenuOpen(false); }}>Duplicate</button>
            <button onClick={e => { e.stopPropagation(); onArchive(!session.archived); setMenuOpen(false); }}>
              {session.archived ? 'Unarchive' : 'Archive'}
            </button>
            <button className="danger" onClick={e => { e.stopPropagation(); onDelete(); setMenuOpen(false); }}>Delete</button>
          </div>
        )}
      </div>

      {/* Swipe affordances — match original */}
      <div className="session-swipe-affordance session-swipe-affordance-right" aria-hidden="true">
        <span className="session-swipe-action-stack">
          <span className="session-swipe-badge" onClick={onArchive.bind(null, !session.archived)}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="4" rx="1"/><path d="M5 8v11a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8"/><path d="M10 12h4"/></svg>
          </span>
          <span className="session-swipe-label">{session.archived ? 'Unarchive' : 'Archive'}</span>
        </span>
      </div>
      <div className="session-swipe-affordance session-swipe-affordance-left" aria-hidden="true">
        <span className="session-swipe-action-stack">
          <span className="session-swipe-badge" onClick={onDelete}>
          ...
          <span className="session-swipe-label">Delete</span>
        </span>
      </div>
    </div>
  );
